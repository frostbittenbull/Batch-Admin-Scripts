...
echo Выполняется добавление подпункта "Перезапустить проводник":
reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\4.restartexplorer" /v "Icon" /t "REG_SZ" /d "explorer.exe" /f
reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\4.restartexplorer" /v "MUIVerb" /t "REG_SZ" /d "Перезапустить проводник" /f
reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\4.restartexplorer\command" /ve /t "REG_EXPAND_SZ" /d "cmd.exe /c taskkill /f /im explorer.exe & start explorer.exe" /f
echo.
...
echo Выполняется добавление подпункта "Перезагрузка в BIOS":
::reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\7.rebootbios" /v "Icon" /t "REG_SZ" /d "shell32.dll,238" /f
::reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\7.rebootbios" /v "MUIVerb" /t "REG_SZ" /d "Перезагрузка в BIOS" /f
::reg add "HKEY_CLASSES_ROOT\DesktopBackground\Shell\instruments\shell\7.rebootbios\command" /ve /t "REG_EXPAND_SZ" /d "shutdown.exe /r /fw /t 0" /f
echo.
